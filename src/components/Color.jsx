...
const Color = ({ onFilterChange }) => {

    const [checkboxes, setCheckboxes] = useState([
        { label: 'Beyaz', checked: false },
        { label: 'Siyah', checked: false },
        { label: 'Gri', checked: false },
        { label: 'Mavi', checked: false },
        { label: 'Kırmızı', checked: false },
        { label: 'Yeşil', checked: false }
    ]);
    ...
    const handleChange = (index) => (event) => {
        ...
        onFilterChange({ color: selectedBrands });
    };
    ...
    return (
        <Grid item md={12} >
            <List
                sx={{ width: '100%', maxWidth: 360, bgcolor: 'f5f5f5', borderBottom: '1px solid black', borderTop: '1px solid black' }}
                component="nav"
            ...
            >
                <ListItemButton onClick={handleClick}>
                    <ListItemText
                        primary="Renk"
                        primaryTypographyProps={{ style: { fontSize: '12.8px' } }}
                    />
